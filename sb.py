...
get_info = False
#scales elements in the menu; form-factor = 0 <=> center aligned text
form_factor_menu = 12
#terminalsize (in chars)
t_width = 0
#menu padding (left, quite frequently used)
ml = ''
#menu padding (right, seldom used)
mr = ''
#how often should package infos be refreshed
refresh_intervall = 0
#whether we support color-output
colour_support = 0
#we aggregate important messages through this string
menutxt=''
#makes a wider range of information available 
dbg=True
#how do we currently control the program, via flag or via menu?
menu_ctrl=False
#this option isn't available through mem.txt or the menu-settings, it wasn't discarded for legacy reasons
...
def evaluate_paths():
    timestart = time.time()
    global initm, SPACK_XPTH, CONFIG_TO_DAT_XPTH, LOC, PROJECT_PTH  
...
def print_menu(txt = ''):
    ...
    if dbg:
        info='\n'
        info+=FCOL[0]+'subshell-use   <=> $>>cmd<<       (Debugging)'+FEND+'\n'
        info+=FCOL[0]+'code eval.     <=> code:>>code<<  (Debugging)'+FEND+'\n'
        info+='\n\n'
